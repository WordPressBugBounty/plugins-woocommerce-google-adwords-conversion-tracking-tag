		...
		$catalog = [
			'google_ads'    => [
				...
				'settings' => [
					...
					'data_manager.operating_account_id'      => [
						'path'        => 'google.ads.data_manager.operating_account_id',
						'type'        => 'string',
						'label'       => 'Conversion upload Google Ads customer ID',
						'description' => 'Experimental. The Google Ads customer ID that owns the conversion action, 8 to 12 digits. Normally written automatically by the Google connect assistant; the server-side conversion upload activates once this, the conversion action ID and the broker enrollment are all present.',
						'advanced'    => true,
						'benefit'     => 'Identifies the Google Ads account that receives the uploaded conversions.',
					],
					'data_manager.login_account_id'          => [
						'path'        => 'google.ads.data_manager.login_account_id',
						'type'        => 'string',
						'label'       => 'Conversion upload manager account ID',
						'description' => 'Experimental. Optional manager (MCC) account customer ID used for access, 8 to 12 digits. Normally written automatically by the Google connect assistant.',
						'advanced'    => true,
						'benefit'     => 'Allows access through a manager account instead of the operating account.',
					],
					'data_manager.conversion_action_id'      => [
						'path'        => 'google.ads.data_manager.conversion_action_id',
						'type'        => 'string',
						'label'       => 'Conversion upload conversion action ID',
						'description' => 'Experimental. The numeric ID of the web conversion action the browser tag fires - uploads go into the same action and Google deduplicates by transaction ID. Normally written automatically by the Google connect assistant.',
						'advanced'    => true,
						'benefit'     => 'Targets the uploads at the conversion action the browser tag fires.',
					],
				],
			],
			'ga4'           => [
			...
			'facebook'      => [
				...
				'settings' => [
					...
					'capi.send_fb_login_id'  => [
						'path'        => 'facebook.capi.send_fb_login_id',
						'type'        => 'boolean',
						'label'       => 'Send Facebook Login ID',
						'description' => 'Sends the Facebook app-scoped user ID (fb_login_id) with Conversions API events for customers who signed in with Facebook. Requires one of the supported social login plugins: Nextend Social Login, miniOrange Social Login, UsersWP Social Login, Super Socializer, Wapu Auth, Heateor Login or Easy Social Login.',
						'advanced'    => true,
						'benefit'     => 'Adds a deterministic Meta identifier, which can improve event match quality. Only matches when the social login plugin\'s Facebook app is in the same Business Manager as the pixel.',
					],
				],
			],
			'bing'          => [
				...
				'settings' => [
					...
					'capi.token'            => [
						'path'        => 'bing.capi.token',
						'type'        => 'string',
						'label'       => 'Conversions API token',
						'description' => 'Microsoft Advertising Conversions API bearer token. Generated in Microsoft Advertising under Tools > UET tag > Set up tagging > Use Conversions API.',
						'secret'      => true,
						'advanced'    => true,
						'benefit'     => 'Enables server-side tracking through the Microsoft Advertising Conversions API, which recovers conversions lost to ad blockers and browser restrictions.',
					],
			...
			'openai'        => [
				...
				'settings' => [
					...
					'advanced_matching' => [
						...
						'type'        => 'boolean',
						'label'       => 'Advanced matching',
						'description' => 'Sends hashed customer identifiers (email, phone, first and last name, customer ID) plus the billing country, region, city and postal code to OpenAI with each event.',
						'advanced'    => true,
			...
			'mixpanel'      => [
				...
				'settings' => [
					'project_token'         => [
						'path'        => 'pixels.mixpanel.project_token',
						'type'        => 'string',
						'label'       => 'Project token',
						'description' => 'Mixpanel project token, a 32 character hexadecimal string. Found in Mixpanel under Settings > Project Settings > Project Token.',
						'required'    => true,
					],
					'data_residency'        => [
						'path'        => 'pixels.mixpanel.data_residency',
						'type'        => 'string',
						'label'       => 'Data residency',
						'description' => 'The region the Mixpanel project is hosted in: us (default), eu or in. Mixpanel does not ingest events sent to the wrong region, so this must match the project. EU projects have a Mixpanel URL starting with eu.mixpanel.com, Indian projects with in.mixpanel.com.',
						'enum'        => [ 'us', 'eu', 'in' ],
					],
					'session_recording'     => [
						'path'        => 'pixels.mixpanel.session_recording',
						'type'        => 'boolean',
						'label'       => 'Session replay and heatmaps',
						'description' => 'Records browsing sessions and heatmap data through the Mixpanel SDK. Session replay must also be enabled in the Mixpanel project.',
						'advanced'    => true,
						'benefit'     => 'Lets you watch how customers move through the shop and where they drop out of the funnel.',
					],
					'autocapture'           => [
						'path'        => 'pixels.mixpanel.autocapture',
						'type'        => 'boolean',
						'label'       => 'Autocapture',
						'description' => 'Lets Mixpanel capture clicks, form submissions and input changes automatically, on top of the e-commerce events the Pixel Manager sends.',
						'advanced'    => true,
						'benefit'     => 'Captures interactions that have no dedicated e-commerce event, without adding tracking code.',
					],
					'user_identification'   => [
						'path'        => 'pixels.mixpanel.user_identification',
						'type'        => 'boolean',
						'label'       => 'User identification',
						'description' => 'Identifies logged-in customers in Mixpanel by their WordPress user ID and writes their email address, name and phone number to the Mixpanel user profile.',
						'advanced'    => true,
						'benefit'     => 'Stitches a customer\'s sessions across devices into one Mixpanel user, which makes retention and cohort reports accurate.',
					],
					'ingestion_api.enabled' => [
						'path'        => 'pixels.mixpanel.ingestion_api.enabled',
						'type'        => 'boolean',
						'label'       => 'Ingestion API',
						'description' => 'Sends purchases and refunds to Mixpanel server-side through the Mixpanel Ingestion API. Authenticates with the project token, so no extra credential is needed.',
						'advanced'    => true,
						'benefit'     => 'Enables server-side tracking, which recovers purchases lost to ad blockers and browser restrictions, and reports refunds Mixpanel would otherwise never see.',
					],
				],
			],
			'klaviyo'       => [
